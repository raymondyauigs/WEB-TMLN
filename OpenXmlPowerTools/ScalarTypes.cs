...
namespace OpenXmlPowerTools
{
    ...
    {
        internal static bool CompareBool(string target,string parmvalue,bool isequal=true)
        {
            if (target == null || parmvalue == null)
                return false;

            if (isequal)
                return target.Trim().ToUpper() == parmvalue.Trim().ToUpper();
            else
                return target.Trim().ToUpper() != parmvalue.Trim().ToUpper();
