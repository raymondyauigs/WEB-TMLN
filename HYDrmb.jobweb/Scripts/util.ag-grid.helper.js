(function (gridHelper, $, uiLb, gridLb) {
    ...
    function createGridColumns(hidefields, columnselector, hidefilterclass, sorterattr, optionsattr, skipclass, fieldidattr, widthattr, wtipattr,formatattr, leastwidth) {
        let gridColumns = [];
        if (!columnselector) {
        ...
        $(columnselector).each(function (i, v) {
            ...
            var ftyper = $(v).attr(formatattr);
            var valformatter = function (p) {
                if (p && !ftyper) {
                    return p.value;
                }
                else if (p) {
            ...
            var cwidth = leastwidth;
            if ($(v).attr(widthattr)) {
                cwidth = parseInt($(v).attr(widthattr));
            }
            ...
            if ($(v).attr(wtipattr)) {
                col['tooltipValueGetter'] = function (params) {
                    return params.value;
                }
            }
